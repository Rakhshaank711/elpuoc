...
function YunufTable({ state, loading, mutate, leave, endRoom, error, loadLog }: GameProps) {
  ...
  const [motions, setMotions] = useState<CardMotion[]>([]); const motionSequence = useRef(0); const previousState = useRef<YunufViewState | null>(null); const playerStacks = useRef(new Map<string, HTMLDivElement>()); const handCards = useRef(new Map<string, HTMLButtonElement>()); const handAreaRef = useRef<HTMLDivElement>(null); const deckRef = useRef<HTMLButtonElement>(null); const discardRef = useRef<HTMLDivElement>(null); const previousDiscardRef = useRef<HTMLButtonElement>(null); const handDrag = useRef<{ cardId: string; pointerId: number; startX: number; startY: number; offsetX: number; offsetY: number; dragging: boolean } | null>(null); const suppressCardClick = useRef(false); const flipPositions = useRef<Map<string, number> | null>(null); const reorderAnimations = useRef(new Map<string, Animation>()); const landingTimer = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    else if (exposedCard) animateTransfer([exposedCard], false, "draw", previousDiscardRef.current ?? discardRef.current, playerStacks.current.get(actorId) ?? null, opponentDiscard ? 360 : 0);
  ...
  return <YunufShell className={`flex h-dvh min-h-dvh flex-col overflow-hidden transition-colors duration-500 ${yourTurn ? "yunuf-your-turn" : "yunuf-waiting-turn"}`}><header className="relative z-10 border-b border-white/[.06] bg-black/15 px-4 pb-3 pt-3"><div className="grid grid-cols-[88px_1fr_88px] items-center"><div className="flex"><button aria-label="Leave match" onClick={() => { if (window.confirm("Leave this Yunuf match? You can rejoin with the same link.")) leave(); }} className="yunuf-icon-button"><LogOut size={14}/></button>{state.you.id === state.hostPlayerId && <button aria-label="End room" disabled={loading} onClick={endRoom} className="yunuf-icon-button border-red-300/15 text-red-200/55 disabled:opacity-35"><Power size={14}/></button>}</div><div className="text-center"><div className="yunuf-eyebrow">Hand {state.handNumber}</div><div className="mt-0.5 text-[9px] text-white/35">{state.completedRounds}/3 rounds before Show</div></div><div className="flex justify-end gap-1"><button aria-label="Open game log" onClick={() => void openLog()} className="yunuf-icon-button"><ScrollText size={14}/></button><button aria-label={muted ? "Turn sound on" : "Mute sound"} onClick={() => setMuted(!muted)} className="yunuf-icon-button">{muted ? <VolumeX size={14}/> : <Volume2 size={14}/>}</button></div></div></header>
    ...
    <div className={`min-h-0 flex-1 overflow-y-auto px-4 pb-3 pt-3 ${yourTurn ? "" : "yunuf-waiting-board"}`}><PlayerRail state={state} registerStack={(playerId, element) => { if (element) playerStacks.current.set(playerId, element); else playerStacks.current.delete(playerId); }}/><div className={`mt-3 flex items-center justify-between rounded-xl border px-3 py-2.5 transition ${yourTurn ? "border-[#d7b45a]/55 bg-[#d7b45a]/15 shadow-[0_0_28px_rgba(215,180,90,.12)]" : "border-white/[.06] bg-black/20"}`}><span className={`flex items-center gap-1.5 text-[10px] font-black ${yourTurn ? "text-[#f4d982]" : "text-white/45"}`}>{yourTurn && <Sparkles size={12}/>} {yourTurn ? "YOUR TURN · PLAY NOW" : `${activePlayerName.toUpperCase()} IS PLAYING`}</span><TurnClock startedAt={state.turnStartedAt} seconds={showDecisionActive ? SHOW_DECISION_SECONDS : state.turnDurationSeconds}/></div>
      <div className="mt-4 grid grid-cols-2 gap-5 px-5"><button ref={deckRef} aria-label="Draw from face-down deck" disabled={!yourTurn || state.turnPhase !== "draw" || loading} onClick={async () => { animateTransfer([undefined], true, "draw", deckRef.current, handAreaRef.current, 0, true); beginIncomingDraw(undefined, true); await mutate({ action: "draw_deck" }); finishIncomingDraw(); }} className="group rounded-xl py-2 text-center transition disabled:opacity-45 enabled:bg-[#d7b45a]/[.055] enabled:ring-1 enabled:ring-[#d7b45a]/25 enabled:active:scale-[.98]"><div className="card-back mx-auto grid h-[96px] w-[68px] place-items-center rounded-lg border-2 border-[#d7b45a]/25"><Layers3 size={20} className="text-[#d7b45a]"/></div><div className="mt-2 text-[9px] font-black text-white/45">FACE-DOWN · {visibleDrawPileCount}</div></button><div ref={discardRef} aria-label={latestDiscardCard ? `Current discard: ${latestDiscardCard.rank} of ${latestDiscardCard.suit}` : "Current discard pile"} className="rounded-xl py-2 text-center"><div className="discard-top-card relative mx-auto h-[96px] w-[68px]">{latestDiscardCard && <PlayingCard card={latestDiscardCard} small/>}</div><div className="mt-2 text-[9px] font-black text-white/45">{state.turnPhase === "draw" ? "JUST DISCARDED" : "TOP DISCARD"}</div></div></div>
      {state.turnPhase === "draw" && previousTopCard && <button ref={previousDiscardRef} aria-label={`Draw top discard: ${previousTopCard.rank} of ${previousTopCard.suit}`} disabled={!yourTurn || loading} onClick={async (event) => { animateTransfer([previousTopCard], false, "draw", event.currentTarget, handAreaRef.current, 0, true); beginIncomingDraw(previousTopCard, false); await mutate({ action: "draw_discard", cardId: previousTopCard.id }); finishIncomingDraw(); }} className="mx-auto mt-3 flex min-h-[82px] w-[min(100%,310px)] items-center gap-4 rounded-xl border border-[#d7b45a]/25 bg-[#d7b45a]/[.07] px-4 py-2 text-left transition disabled:border-white/[.07] disabled:bg-black/15 enabled:ring-1 enabled:ring-[#d7b45a]/20 enabled:active:scale-[.985]"><div className="relative h-[64px] w-[44px] shrink-0">{displayedPreviousCard && <PlayingCard card={displayedPreviousCard} tiny/>}</div><div><div className="text-[8px] font-black tracking-[.12em] text-[#e8cf8a]">PREVIOUS TOP · SAFE TO TAKE</div><div className="mt-1 text-[10px] font-black text-white/75">{yourTurn ? `Take ${previousTopCard.rank} of ${previousTopCard.suit}` : `${activePlayerName} may take this card`}</div><p className="mt-1 text-[8px] leading-3 text-white/35">The new discard stays on the table.</p></div></button>}
      {yourTurn && state.turnPhase === "draw" && <div className="mt-3 text-center"><div className="text-[9px] font-black text-[#e8cf8a]">DRAW ONE CARD</div><p className="mt-1 text-[8px] text-white/35">Choose the face-down deck or the reserved previous top.</p></div>}
    ...
    <div className="border-t border-white/[.07] bg-[#121a18]/95 px-4 pb-[max(12px,env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl">{yourTurn && state.turnPhase === "discard" && (arrangeMode ? <div className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-[#d7b45a]/20 bg-[#d7b45a]/8 text-[10px] font-bold text-[#e8cf8a]"><GripVertical size={14}/>Arranging hand · tap Done above to select cards</div> : <><div className={`mb-2 text-center text-[9px] font-bold ${activeSelected.length && validation.valid ? "text-emerald-200" : "text-white/35"}`}>{!activeSelected.length ? "Select in landing order · your last pick stays on top" : validation.valid ? `Valid ${validation.playType} · ${topSelectedCard?.rank} will stay on top` : validation.error}</div><YunufButton disabled={loading || !validation.valid} onClick={async () => { const cardIds = [...activeSelected]; animateTransfer(selectedCards, false, "discard", selectedCards.map((card) => handCards.current.get(card.id) ?? null), discardRef.current, 0, true); setDeparting(cardIds); const result = await mutate({ action: "discard", cardIds }); setDeparting([]); if (result) setSelected([]); }}><ArrowDownToLine size={15}/>Discard {activeSelected.length || "cards"}{topSelectedCard ? ` · ${topSelectedCard.rank} on top` : ""}</YunufButton></>)}
      {yourTurn && state.turnPhase === "draw" && <div className="text-center text-[10px] font-bold text-[#e8cf8a]">Choose the deck or the reserved previous card above</div>}
