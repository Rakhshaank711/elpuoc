...
import {
  ...
  Crown, Eye, GripVertical, House, Layers3, LogOut, Play, Power, RotateCcw, Send, Sparkles,
...
type CardMotion = { id: string; card: Card | undefined; faceDown: boolean; large: boolean; kind: "discard" | "draw" | "merge"; from: MotionPoint; via: MotionPoint; to: MotionPoint; delay: number; rotation: number };
...
export function YunufApp() {
  ...
  const [roomEndedBy, setRoomEndedBy] = useState<string | null>(null);
  ...
  const fetchState = useCallback(async (active: YunufSession, quiet = false) => {
    try {
      ...
      if (sessionRef.current?.playerId !== active.playerId || sessionRef.current.token !== active.token) return;
      if (cause instanceof ApiError && cause.status === 404) { setRoomEndedBy("The host"); setSyncError(null); setError(null); return; }
  ...
  useEffect(() => {
    ...
    channel.on("broadcast", { event: "state_changed" }, ({ payload }) => {
      ...
      .on("broadcast", { event: "room_ended" }, ({ payload }) => {
        setRoomEndedBy(typeof payload?.actorName === "string" ? payload.actorName : "The host");
  ...
  const endRoom = useCallback(async () => {
    if (!session || !stateRef.current || stateRef.current.hostPlayerId !== session.playerId) return;
    if (!window.confirm("End this room for everyone? This permanently removes the room and its game history.")) return;
    setLoading(true); setError(null);
    try {
      await api<{ ended: true }>("/api/yunuf", { method: "POST", headers: { "content-type": "application/json", "x-player-id": session.playerId, "x-player-token": session.token }, body: JSON.stringify({ action: "end_room", code: session.code, expectedVersion: stateRef.current.version, actionId: crypto.randomUUID() }) });
      localStorage.removeItem(sessionKey(session.code));
      setRoomEndedBy("You");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Could not end this room.");
      if (cause instanceof ApiError && cause.status === 404) {
        localStorage.removeItem(sessionKey(session.code));
        setRoomEndedBy("You");
        setError(null);
      } else if (cause instanceof ApiError && cause.status === 409 && await fetchState(session, true)) setError(null);
    } finally { setLoading(false); }
  }, [fetchState, session]);
  const leave = () => { sessionRef.current = null; stateRef.current = null; syncQueuedSessionRef.current = null; setSession(null); setState(null); setError(null); setSyncError(null); setRoomEndedBy(null); router.push("/"); };
  if (booting) return <YunufShell><div className="grid min-h-dvh place-items-center"><YunufMark/></div></YunufShell>;
  if (roomEndedBy && session) return <YunufRoomEnded endedBy={roomEndedBy} onHome={() => { localStorage.removeItem(sessionKey(session.code)); leave(); }}/>;
  if (!session || !state) {
  ...
  if (state.status === "lobby") return <YunufLobby state={connectedState} loading={loading} error={visibleError} mutate={mutate} leave={leave} endRoom={endRoom}/>;
  if (state.status === "hand_results") return <div className="relative"><YunufResults state={connectedState} loading={loading} mutate={mutate} leave={leave} endRoom={endRoom}/><GameLogLauncher state={connectedState} loadLog={loadLog}/></div>;
  if (state.status === "match_over") return <div className="relative"><YunufMatchOver state={connectedState} loading={loading} mutate={mutate} leave={leave} endRoom={endRoom}/><GameLogLauncher state={connectedState} loadLog={loadLog}/></div>;
  return <YunufTable state={connectedState} loading={loading} error={visibleError} mutate={mutate} leave={leave} endRoom={endRoom} loadLog={loadLog}/>;
}
...
function YunufRoomEnded({ endedBy, onHome }: { endedBy: string; onHome: () => void }) { return <YunufShell className="flex min-h-dvh flex-col px-6 pb-8 pt-8"><div className="flex justify-center"><YunufMark/></div><div className="flex flex-1 flex-col items-center justify-center text-center"><div className="grid size-24 place-items-center rounded-full border border-red-300/20 bg-red-400/10 shadow-[0_0_55px_rgba(248,113,113,.12)]"><Power size={34} className="text-red-200"/></div><div className="yunuf-eyebrow mt-7">Room closed</div><h1 className="mt-3 text-[30px] font-black">This table has ended.</h1><p className="mt-3 max-w-xs text-[11px] leading-5 text-white/45">{endedBy === "You" ? "You ended the room for everyone." : `${endedBy} ended the room.`} The temporary room and game history have been removed.</p></div><YunufButton onClick={onHome}><House size={15}/>Back to all games</YunufButton></YunufShell>; }
...
type GameProps = { state: YunufViewState; loading: boolean; mutate: (payload: Record<string, unknown>) => Promise<YunufViewState | undefined>; leave: () => void; endRoom: () => void; error?: string | null; loadLog?: () => Promise<YunufGameEvent[]> };

function YunufLobby({ state, loading, mutate, leave, endRoom, error }: GameProps) {
  ...
  return <YunufShell className="px-5 pb-8 pt-7"><div className="flex items-center justify-between"><button aria-label="Leave room" onClick={leave} className="yunuf-icon-button"><House size={15}/></button><YunufMark/><span className="rounded-full border border-white/10 px-2.5 py-1 text-[9px] font-bold text-white/40">{state.players.length}/5</span></div><div className="mt-8 text-center"><div className="yunuf-eyebrow">Your private table</div><h1 className="mt-3 text-[28px] font-black">Waiting for players</h1><div className="mx-auto mt-5 w-fit rounded-xl border border-[#d7b45a]/20 bg-[#d7b45a]/8 px-8 py-4"><span className="text-[8px] font-black tracking-[.14em] text-white/35">ROOM CODE</span><div className="mt-1 text-[25px] font-black tracking-[.14em] text-[#e8cf8a]">{state.code}</div></div></div>
    ...
    <div className="mt-5">{isHost ? <YunufButton disabled={loading || !allReady} onClick={() => mutate({ action: "start" })}><Play size={15}/>{allReady ? "Start match" : state.players.length < 2 ? "Invite at least one player" : "Waiting for everyone"}</YunufButton> : <YunufButton secondary disabled={loading || you.ready} onClick={() => mutate({ action: "ready", ready: true })}>{you.ready ? <><Check size={15}/>You’re ready</> : "Ready up"}</YunufButton>}{isHost && !you.ready && <button onClick={() => mutate({ action: "ready", ready: true })} className="mt-3 min-h-11 w-full text-[10px] font-bold text-white/45">Mark myself ready</button>}</div>{error && <YunufError message={error}/>} {isHost && <button aria-label="End room" disabled={loading} onClick={endRoom} className="mt-5 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-red-300/15 bg-red-400/[.055] text-[9px] font-black text-red-200/65 transition hover:bg-red-400/10 disabled:opacity-35"><Power size={13}/>End room for everyone</button>}</YunufShell>;
}

function YunufTable({ state, loading, mutate, leave, endRoom, error, loadLog }: GameProps) {
  ...
  const [motions, setMotions] = useState<CardMotion[]>([]); const motionSequence = useRef(0); const previousState = useRef<YunufViewState | null>(null); const playerStacks = useRef(new Map<string, HTMLDivElement>()); const handCards = useRef(new Map<string, HTMLButtonElement>()); const handAreaRef = useRef<HTMLDivElement>(null); const deckRef = useRef<HTMLButtonElement>(null); const discardRef = useRef<HTMLButtonElement>(null); const currentDiscardRef = useRef<HTMLDivElement>(null); const handDrag = useRef<{ cardId: string; pointerId: number; startX: number; startY: number; offsetX: number; offsetY: number; dragging: boolean } | null>(null); const suppressCardClick = useRef(false); const flipPositions = useRef<Map<string, number> | null>(null); const reorderAnimations = useRef(new Map<string, Animation>()); const landingTimer = useRef<number | null>(null);
  ...
  const validation = validateDiscard(selectedCards); const topSelectedCard = selectedCards.at(-1); const previousTopCard = state.drawSourceDiscard?.cards.at(-1) ?? null; const latestDiscardCard = state.latestDiscard?.cards.at(-1) ?? null; const topDrawIsPending = Boolean(incomingDraw?.card && incomingDraw.card.id === previousTopCard?.id); const displayedPreviousCard = topDrawIsPending ? state.drawSourceDiscard?.cards.at(-2) ?? null : previousTopCard; const tableDiscardCard = state.turnPhase === "draw" ? displayedPreviousCard : latestDiscardCard; const currentDiscardCards = state.turnPhase === "draw" ? state.latestDiscard?.cards ?? [] : []; const visibleDrawPileCount = Math.max(0, state.drawPileCount - (incomingDraw?.faceDown ? 1 : 0)); const visibleHandValue = calculateHandValue(hand) + (incomingDraw?.landed && incomingDraw.card ? getCardValue(incomingDraw.card.rank) : 0);
  ...
  useEffect(() => {
    ...
    if (deckDrawn) animateTransfer([undefined], true, "draw", deckRef.current, playerStacks.current.get(actorId) ?? null, 0);
    else if (exposedCard) animateTransfer([exposedCard], false, "draw", discardRef.current, playerStacks.current.get(actorId) ?? null, 0);
    animateTransfer(before.latestDiscard?.cards ?? [], false, "merge", currentDiscardRef.current, discardRef.current, 190, true);
  ...
  return <YunufShell className={`flex h-dvh min-h-dvh flex-col overflow-hidden transition-colors duration-500 ${yourTurn ? "yunuf-your-turn" : "yunuf-waiting-turn"}`}><header className="relative z-10 border-b border-white/[.06] bg-black/15 px-4 pb-3 pt-3"><div className="grid grid-cols-[88px_1fr_88px] items-center"><div className="flex"><button aria-label="Leave match" onClick={() => { if (window.confirm("Leave this Yunuf match? You can rejoin with the same link.")) leave(); }} className="yunuf-icon-button"><LogOut size={14}/></button>{state.you.id === state.hostPlayerId && <button aria-label="End room" disabled={loading} onClick={endRoom} className="yunuf-icon-button border-red-300/15 text-red-200/55 disabled:opacity-35"><Power size={14}/></button>}</div><div className="text-center"><div className="yunuf-eyebrow">Hand {state.handNumber}</div><div className="mt-0.5 text-[9px] text-white/35">{state.completedRounds}/3 rounds before Show</div></div><div className="flex justify-end gap-1"><button aria-label="Open game log" onClick={() => void openLog()} className="yunuf-icon-button"><ScrollText size={14}/></button><button aria-label={muted ? "Turn sound on" : "Mute sound"} onClick={() => setMuted(!muted)} className="yunuf-icon-button">{muted ? <VolumeX size={14}/> : <Volume2 size={14}/>}</button></div></div></header>
    ...
    <div className={`min-h-0 flex-1 overflow-y-auto px-4 pb-3 pt-3 ${yourTurn ? "" : "yunuf-waiting-board"}`}><PlayerRail state={state} registerStack={(playerId, element) => { if (element) playerStacks.current.set(playerId, element); else playerStacks.current.delete(playerId); }}/><div className={`mt-3 flex items-center justify-between rounded-xl border px-3 py-2.5 transition ${yourTurn ? "border-[#d7b45a]/55 bg-[#d7b45a]/15 shadow-[0_0_28px_rgba(215,180,90,.12)]" : "border-white/[.06] bg-black/20"}`}><span className={`flex items-center gap-1.5 text-[10px] font-black ${yourTurn ? "text-[#f4d982]" : "text-white/45"}`}>{yourTurn && <Sparkles size={12}/>} {yourTurn ? "YOUR TURN · PLAY NOW" : `${activePlayerName.toUpperCase()} IS PLAYING`}</span><TurnClock startedAt={state.turnStartedAt} seconds={showDecisionActive ? SHOW_DECISION_SECONDS : state.turnDurationSeconds}/></div>
      <div className="mx-auto mt-4 flex w-fit items-start gap-2 px-2"><button ref={deckRef} aria-label="Draw from face-down deck" disabled={!yourTurn || state.turnPhase !== "draw" || loading} onClick={async () => { animateTransfer([undefined], true, "draw", deckRef.current, handAreaRef.current, 0, true); animateTransfer(currentDiscardCards, false, "merge", currentDiscardRef.current, discardRef.current, 190, true); beginIncomingDraw(undefined, true); await mutate({ action: "draw_deck" }); finishIncomingDraw(); }} className="group w-[82px] rounded-xl py-2 text-center transition disabled:opacity-45 enabled:bg-[#d7b45a]/[.055] enabled:ring-1 enabled:ring-[#d7b45a]/25 enabled:active:scale-[.98]"><div className="card-back mx-auto grid h-[96px] w-[68px] place-items-center rounded-lg border-2 border-[#d7b45a]/25"><Layers3 size={20} className="text-[#d7b45a]"/></div><div className="mt-2 text-[9px] font-black text-white/45">FACE-DOWN · {visibleDrawPileCount}</div></button><div className="relative h-[128px] w-[150px]"><button ref={discardRef} aria-label={previousTopCard && state.turnPhase === "draw" ? `Draw top discard: ${previousTopCard.rank} of ${previousTopCard.suit}` : tableDiscardCard ? `Top discard: ${tableDiscardCard.rank} of ${tableDiscardCard.suit}` : "Top discard pile"} disabled={!yourTurn || state.turnPhase !== "draw" || loading || !previousTopCard} onClick={async (event) => { if (!previousTopCard) return; animateTransfer([previousTopCard], false, "draw", event.currentTarget, handAreaRef.current, 0, true); animateTransfer(currentDiscardCards, false, "merge", currentDiscardRef.current, event.currentTarget, 190, true); beginIncomingDraw(previousTopCard, false); await mutate({ action: "draw_discard", cardId: previousTopCard.id }); finishIncomingDraw(); }} className="absolute left-0 top-0 w-[82px] rounded-xl py-2 text-center transition disabled:opacity-100 enabled:bg-[#d7b45a]/[.055] enabled:ring-1 enabled:ring-[#d7b45a]/25 enabled:active:scale-[.98]"><div className="discard-top-card relative mx-auto h-[96px] w-[68px]">{tableDiscardCard && <PlayingCard card={tableDiscardCard} small/>}</div><div className="mt-2 text-[9px] font-black text-white/45">{state.turnPhase === "draw" ? "DRAWABLE TOP" : "TOP DISCARD"}</div></button><div ref={currentDiscardRef} aria-label={currentDiscardCards.length && latestDiscardCard ? `Current discard: ${latestDiscardCard.rank} of ${latestDiscardCard.suit}` : undefined} aria-hidden={!currentDiscardCards.length} className={`absolute left-[72px] top-2 w-[78px] text-center transition-opacity duration-200 ${currentDiscardCards.length ? "opacity-100" : "pointer-events-none opacity-0"}`}><div className="discard-top-card relative mx-auto h-[96px] w-[68px]">{latestDiscardCard && <PlayingCard card={latestDiscardCard} small/>}{currentDiscardCards.length > 1 && <span className="absolute -right-1 -top-2 z-10 rounded-full border border-[#f8f1dc]/25 bg-[#d7b45a] px-1.5 py-1 text-[7px] font-black text-[#17201d]">{currentDiscardCards.length}</span>}</div><div className="mt-2 text-[8px] font-black text-[#e8cf8a]">CURRENT</div></div></div></div>
      {yourTurn && state.turnPhase === "draw" && <div className="mt-2 text-center"><div className="text-[9px] font-black text-[#e8cf8a]">DRAW ONE CARD</div><p className="mt-1 text-[8px] text-white/35">Take the drawable top or choose the face-down deck.</p></div>}
...
function YunufResults({ state, loading, mutate, leave, endRoom }: GameProps) { const result = state.result!; return <YunufShell className="px-5 pb-8 pt-7"><div className="flex justify-between"><button onClick={leave} className="yunuf-icon-button"><House size={15}/></button><YunufMark/><span className="size-11"/></div><div className="mt-7 text-center"><div className="yunuf-eyebrow">Hand {state.handNumber} revealed</div><Trophy className="mx-auto mt-5 text-[#e8cf8a]" size={34}/><h1 className="mt-3 text-[27px] font-black">{result.winnerIds.length > 1 ? "Joint winners!" : `${state.players.find((player) => player.id === result.winnerIds[0])?.name} wins the hand`}</h1><p className="mt-2 text-[10px] text-white/40">{state.players.find((player) => player.id === result.declarerId)?.name} declared Show.</p></div><div className="mt-7 space-y-3">{[...state.players].sort((a,b) => (result.handValues[a.id] ?? 99)-(result.handValues[b.id] ?? 99)).map((player) => <div key={player.id} className={`rounded-xl border p-3 ${result.winnerIds.includes(player.id) ? "border-[#d7b45a]/35 bg-[#d7b45a]/10" : "border-white/[.07] bg-white/[.025]"}`}><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Avatar index={player.avatar} size="sm"/><div><div className="text-[10px] font-black">{player.name}{player.id === result.declarerId ? " · SHOW" : ""}</div><div className="mt-0.5 text-[8px] text-white/35">+{result.roundScores[player.id]} this hand · {player.totalScore} total</div></div></div><div className="text-xl font-black text-[#e8cf8a]">{result.handValues[player.id]}</div></div><div className="mt-3 flex gap-1 overflow-x-auto">{player.hand?.map((card) => <PlayingCard key={card.id} card={card} tiny reveal/>)}</div>{result.eliminatedIds.includes(player.id) && <div className="mt-2 text-[8px] font-black text-red-300">ELIMINATED AT {player.totalScore}</div>}{player.id === result.declarerId && !result.declarerWon && <div className="mt-2 text-[8px] font-black text-red-300">+10 FAILED SHOW PENALTY</div>}</div>)}</div>{state.you.id === state.hostPlayerId ? <><YunufButton className="mt-6" disabled={loading} onClick={() => mutate({ action: "continue" })}><RotateCcw size={14}/>Deal next hand</YunufButton><button aria-label="End room" disabled={loading} onClick={endRoom} className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-red-300/15 bg-red-400/[.055] text-[9px] font-black text-red-200/65 transition hover:bg-red-400/10 disabled:opacity-35"><Power size={13}/>End room for everyone</button></> : <p className="mt-6 text-center text-[10px] text-white/35">Waiting for the host to deal…</p>}</YunufShell>; }

function YunufMatchOver({ state, loading, mutate, leave, endRoom }: GameProps) { const winners = state.result?.matchWinnerIds ?? []; return <YunufShell className="flex flex-col px-6 pb-8 pt-8"><button onClick={leave} className="yunuf-icon-button"><House size={15}/></button><div className="flex flex-1 flex-col justify-center text-center"><div className="relative mx-auto grid size-28 place-items-center rounded-full border border-[#d7b45a]/35 bg-[#d7b45a]/10 shadow-[0_0_70px_rgba(215,180,90,.25)]"><Crown size={42} className="text-[#e8cf8a]"/><Sparkles className="absolute -right-1 top-1 text-[#e8cf8a]"/></div><div className="yunuf-eyebrow mt-8">Match complete</div><h1 className="mt-3 text-[34px] font-black">{winners.length > 1 ? "Joint champions" : `${state.players.find((player) => player.id === winners[0])?.name} wins Yunuf`}</h1><div className="mt-6 space-y-2">{[...state.players].sort((a,b) => a.totalScore-b.totalScore).map((player) => <div key={player.id} className="rounded-xl border border-white/[.07] bg-white/[.025] px-4 py-3"><div className="flex items-center justify-between"><span className="text-[11px] font-black">{player.name}</span><span className="text-[11px] font-black text-[#e8cf8a]">{player.totalScore} pts</span></div><div className="mt-1 text-left text-[8px] text-white/30">{player.handsWon} wins · {player.showsDeclared} Shows · {player.failedShows} failed</div></div>)}</div></div><div className="grid grid-cols-2 gap-2">{state.you.id === state.hostPlayerId ? <YunufButton onClick={() => mutate({ action: "reset" })} disabled={loading}><RotateCcw size={14}/>Play again</YunufButton> : <div className="grid place-items-center text-[9px] text-white/35">Host can restart</div>}{state.you.id === state.hostPlayerId ? <YunufButton secondary onClick={endRoom} disabled={loading}><Power size={14}/>End room</YunufButton> : <YunufButton secondary onClick={leave} disabled={loading}>Leave room</YunufButton>}</div></YunufShell>; }
