...
function YunufTable({ state, loading, mutate, leave, error }: GameProps) {
  ...
  return <YunufShell className="flex h-dvh min-h-dvh flex-col overflow-hidden"><header className="border-b border-white/[.06] bg-black/15 px-4 pb-3 pt-3"><div className="flex items-center justify-between"><button aria-label="Leave match" onClick={() => { if (window.confirm("Leave this Yunuf match? You can rejoin with the same link.")) leave(); }} className="yunuf-icon-button"><LogOut size={14}/></button><div className="text-center"><div className="yunuf-eyebrow">Hand {state.handNumber}</div><div className="mt-0.5 text-[9px] text-white/35">{state.completedRounds}/3 rounds before Show</div></div><button aria-label={muted ? "Turn sound on" : "Mute sound"} onClick={() => setMuted(!muted)} className="yunuf-icon-button">{muted ? <VolumeX size={14}/> : <Volume2 size={14}/>}</button></div></header>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-3 pt-3"><PlayerRail state={state}/><div className="mt-3 flex items-center justify-between rounded-xl border border-white/[.06] bg-white/[.025] px-3 py-2"><span className="text-[9px] font-bold text-white/35">{yourTurn ? "YOUR TURN" : `${state.players.find((player) => player.id === state.currentPlayerId)?.name ?? "Table"} IS PLAYING`}</span><TurnClock startedAt={state.turnStartedAt} seconds={state.turnDurationSeconds}/></div>
      <div className="mt-4 grid grid-cols-2 gap-5 px-5"><button disabled={!yourTurn || state.turnPhase !== "draw" || loading} onClick={() => mutate({ action: "draw_deck" })} className="group text-center disabled:opacity-50"><div className="card-back mx-auto grid h-[96px] w-[68px] place-items-center rounded-lg border-2 border-[#d7b45a]/25"><Layers3 size={20} className="text-[#d7b45a]"/></div><div className="mt-2 text-[9px] font-black text-white/45">DRAW PILE · {state.drawPileCount}</div></button><div className="text-center"><div className="relative mx-auto h-[96px] w-[86px]">{state.latestDiscard?.cards.slice(-3).map((card, index) => <span key={card.id} className="absolute" style={{ left: index * 9, transform: `rotate(${(index - 1) * 5}deg)` }}><PlayingCard card={card} small/></span>)}</div><div className="mt-2 text-[9px] font-black text-white/45">LATEST PLAY</div></div></div>
      {yourTurn && state.turnPhase === "draw" && <div className="mt-4 rounded-xl border border-[#d7b45a]/20 bg-[#d7b45a]/[.06] p-3"><div className="text-center text-[9px] font-black text-[#e8cf8a]">DRAW ONE CARD</div><p className="mt-1 text-center text-[8px] text-white/35">Choose the deck or an exposed end from the previous play.</p><div className="mt-3 flex justify-center gap-2">{state.drawSourceDiscard?.cards.filter((card) => sourceIds.includes(card.id)).map((card) => <button key={card.id} aria-label={`Draw ${card.rank} of ${card.suit}`} onClick={() => mutate({ action: "draw_discard", cardId: card.id })}><PlayingCard card={card} tiny/></button>)}</div></div>}
...
function PlayerRail({ state }: { state: YunufViewState }) { return <div className="flex gap-2 overflow-x-auto pb-1">{state.players.map((player) => <div key={player.id} className={`min-w-[104px] rounded-xl border p-2.5 ${player.id === state.currentPlayerId ? "border-[#d7b45a]/45 bg-[#d7b45a]/10" : "border-white/[.06] bg-white/[.025]"}`}><div className="flex items-center gap-2"><Avatar index={player.avatar} size="sm"/><div className="min-w-0"><div className="max-w-14 truncate text-[9px] font-black">{player.id === state.you.id ? "You" : player.name}</div><div className="mt-0.5 text-[7px] text-white/30">{player.cardCount} cards</div></div></div><div className="mt-2 flex items-center justify-between text-[8px]"><span className="text-white/30">Score</span><b className={player.totalScore >= state.eliminationScore * .75 ? "text-red-300" : "text-[#e8cf8a]"}>{player.totalScore}</b></div></div>)}</div>; }
