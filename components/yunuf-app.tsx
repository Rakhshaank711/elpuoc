...
import { calculateHandValue, getCardValue, rankIndex, validateDiscard } from "@/lib/yunuf/rules";
...
function YunufTable({ state, loading, mutate, leave, error }: GameProps) {
  const you = state.players.find((player) => player.id === state.you.id)!; const yourTurn = state.currentPlayerId === you.id; const [selected, setSelected] = useState<string[]>([]); const [departing, setDeparting] = useState<string[]>([]); const [showConfirm, setShowConfirm] = useState(false); const [muted, setMuted] = useState(true); const [order, setOrder] = useState(() => (you.hand ?? []).map((card) => card.id));
  const [motions, setMotions] = useState<CardMotion[]>([]); const motionSequence = useRef(0); const previousState = useRef<YunufViewState | null>(null); const playerStacks = useRef(new Map<string, HTMLDivElement>()); const handCards = useRef(new Map<string, HTMLButtonElement>()); const handAreaRef = useRef<HTMLDivElement>(null); const deckRef = useRef<HTMLButtonElement>(null); const discardRef = useRef<HTMLButtonElement>(null);
  ...
  const validation = validateDiscard(selectedCards); const topSelectedCard = selectedCards.at(-1); const topDiscardCard = state.drawSourceDiscard?.cards.at(-1) ?? null; const displayedDiscardCard = topDiscardCard ?? state.latestDiscard?.cards.at(-1) ?? null;
  ...
  return <YunufShell className="flex h-dvh min-h-dvh flex-col overflow-hidden"><header className="border-b border-white/[.06] bg-black/15 px-4 pb-3 pt-3"><div className="flex items-center justify-between"><button aria-label="Leave match" onClick={() => { if (window.confirm("Leave this Yunuf match? You can rejoin with the same link.")) leave(); }} className="yunuf-icon-button"><LogOut size={14}/></button><div className="text-center"><div className="yunuf-eyebrow">Hand {state.handNumber}</div><div className="mt-0.5 text-[9px] text-white/35">{state.completedRounds}/3 rounds before Show</div></div><button aria-label={muted ? "Turn sound on" : "Mute sound"} onClick={() => setMuted(!muted)} className="yunuf-icon-button">{muted ? <VolumeX size={14}/> : <Volume2 size={14}/>}</button></div></header>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-3 pt-3"><PlayerRail state={state} registerStack={(playerId, element) => { if (element) playerStacks.current.set(playerId, element); else playerStacks.current.delete(playerId); }}/><div className="mt-3 flex items-center justify-between rounded-xl border border-white/[.06] bg-white/[.025] px-3 py-2"><span className="text-[9px] font-bold text-white/35">{yourTurn ? "YOUR TURN" : `${state.players.find((player) => player.id === state.currentPlayerId)?.name ?? "Table"} IS PLAYING`}</span><TurnClock startedAt={state.turnStartedAt} seconds={state.turnDurationSeconds}/></div>
      <div className="mt-4 grid grid-cols-2 gap-5 px-5"><button ref={deckRef} disabled={!yourTurn || state.turnPhase !== "draw" || loading} onClick={() => { animateTransfer([undefined], true, "draw", deckRef.current, handAreaRef.current, 0, true); void mutate({ action: "draw_deck" }); }} className="group rounded-xl py-2 text-center transition disabled:opacity-45 enabled:bg-[#d7b45a]/[.055] enabled:ring-1 enabled:ring-[#d7b45a]/25 enabled:active:scale-[.98]"><div className="card-back mx-auto grid h-[96px] w-[68px] place-items-center rounded-lg border-2 border-[#d7b45a]/25"><Layers3 size={20} className="text-[#d7b45a]"/></div><div className="mt-2 text-[9px] font-black text-white/45">FACE-DOWN · {state.drawPileCount}</div></button><button ref={discardRef} aria-label={topDiscardCard ? `Draw top discard: ${topDiscardCard.rank} of ${topDiscardCard.suit}` : "Top discard"} disabled={!yourTurn || state.turnPhase !== "draw" || loading || !topDiscardCard} onClick={(event) => { if (!topDiscardCard) return; animateTransfer([topDiscardCard], false, "draw", event.currentTarget, handAreaRef.current, 0, true); void mutate({ action: "draw_discard", cardId: topDiscardCard.id }); }} className="group rounded-xl py-2 text-center transition disabled:opacity-55 enabled:bg-[#d7b45a]/[.055] enabled:ring-1 enabled:ring-[#d7b45a]/25 enabled:active:scale-[.98]"><div className="discard-top-card relative mx-auto h-[96px] w-[68px]">{displayedDiscardCard && <PlayingCard card={displayedDiscardCard} small/>}</div><div className="mt-2 text-[9px] font-black text-white/45">{yourTurn && state.turnPhase === "draw" ? "TAKE TOP CARD" : "TOP DISCARD"}</div></button></div>
      ...
      <div ref={handAreaRef} className="mt-3 flex min-h-[130px] items-end overflow-x-auto px-2 pb-2 pt-5">{hand.map((card, index) => { const selectionIndex = activeSelected.indexOf(card.id); const selectedNow = selectionIndex >= 0; const isTop = selectedNow && selectionIndex === activeSelected.length - 1; return <button ref={(element) => { if (element) handCards.current.set(card.id, element); else handCards.current.delete(card.id); }} key={card.id} aria-label={`${card.rank} of ${card.suit}`} aria-pressed={selectedNow} disabled={!yourTurn || state.turnPhase !== "discard"} onClick={() => setSelected((ids) => ids.includes(card.id) ? ids.filter((id) => id !== card.id) : [...ids, card.id])} className={`relative shrink-0 transition-transform ${index ? "-ml-4" : ""} ${departing.includes(card.id) ? "invisible" : ""} ${selectedNow ? "z-20 -translate-y-4" : "hover:-translate-y-1"}`}><PlayingCard card={card} selected={selectedNow}/>{selectedNow && <span className={`absolute -right-1 -top-2 z-30 grid min-w-5 place-items-center rounded-full border px-1 py-1 text-[7px] font-black shadow-lg ${isTop ? "border-[#f8f1dc]/35 bg-[#d7b45a] text-[#17201d]" : "border-white/20 bg-[#17201d] text-[#e8cf8a]"}`}>{isTop ? "TOP" : selectionIndex + 1}</span>}</button>; })}</div>
